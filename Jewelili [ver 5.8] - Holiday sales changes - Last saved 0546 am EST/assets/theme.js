...
slate.Variants = (function() {
  ...
  Variants.prototype = _.assignIn({}, Variants.prototype, {
    ...
    _onSelectChange: function() {
      ...
      // Free product text on PDP
      $('.product-single__meta').attr('data-product-price', variant.price);
      if(variant.price < 19899){
        $('.shipping-text .free-product-text').removeClass('hide');
      }else{
        $('.shipping-text .free-product-text').addClass('hide');
      }
    },
...
theme.MobileNav = (function() {
  ...
  function openMobileNav() {
    ...
    var translateNavContainerWidth = cache.$mobileNavContainer.outerWidth();
...
//       transform:
//         'translateX(' + cache.$mobileNavContainer[0].scrollWidth + 'px)'
//     });
...
//   $('.mobile-nav-wrapper, .mobile-nav--open').click(function(event){
...
theme.Cart = (function() {
  var selectors = {
    ...
    cartLineTotalPrice: '[data-cart-line-total]',
  ...
  function Cart(container) {
    ...
    this.$container.on(
      'click',
      selectors.cartGift,
      $.debounce(500, this._addGift.bind(this))
    );
  ...
  Cart.prototype = _.assignIn({}, Cart.prototype, {
    ...
    _addGift : function(evt){
      var $input = $(evt.target);
      var $itemElement = $input.closest(selectors.cartItem);
      var index = $itemElement.attr(attributes.cartItemIndex);
      let index_cart = ($itemElement.attr(attributes.cartItemIndex))-1;
      let items = $.getJSON('/cart.js',function (data){
        let property = data.items[index_cart].properties;
        let quantity = data.items[index_cart].quantity;
        let params = {};
        if(property){
          if(property.Gift == "false" || property.Gift == undefined || property.Gift == null){
            params = {
              url: '/cart/change.js',
              data: {"quantity": quantity,"properties":{ "Gift": true }, line: index },
              dataType: 'json'
            };
          }
          else{
            params = {
              url: '/cart/change.js',
              data: {"quantity": quantity, "properties":{ 'Gift' : false }, line: index },
              dataType: 'json'
            };
          }
        }
          ...
          function(state) {
            if (state.item_count === 0) {
              ...
              if (value === 0) {
                ...
                this._updateLiveRegion(item,$lineItem);
              }
            }
    ...
    _updateLiveRegion: function(item,lineitem) {
      ...
      $(selectors.cartLineTotalPrice, lineitem)
      .html( theme.Currency.formatMoney(
        item.line_price,
        theme.money
      )
           );
    ...
    _getOptionList: function(options) {
      return $.map(
        ...
        function(option) {
          ...
          $optionElement
            .html(option.name + `<span class="seperator"></span>` + option.value)
            .removeClass(classes.hide);
    ...
    _onRemoveItem: function(evt) {
      ...
      var $remove = $(evt.target);
       var $lineItem = $remove.parents(selectors.cartItem);
    ...
    _getRemoveMessage: function(lineItem) {
      ...
      var $tableCell = $(selectors.errorCell, lineItem).clone();
      $tableCell
        .removeClass()
...
theme.Product = (function() {
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    ...
    _switchMedia: function(mediaId) {
      ...
      //       $('.product-single__thumbnail-image[data-thumbnail-id='+moveToMedia+']').parents('.slick-slide').trigger('click');
    },
    ...
    $.get(recommendationsSectionUrl).then(
      function(section) {
        ...
        if (recommendationsMarkup.trim() !== '') {
          ...
          this.$container.find('h4 span').text(productTitle);
                    
          function sliderInit(){
            $('.mm__recommended--items').slick({
              slidesToShow: 4,
              slidesToScroll: 1,
              infinite: false,
              dots: true,
...
$(document).ready(function() {
  ...
  $(document).on('click','.quantity_modifier', function(){
    let inputVal = parseInt($(this).closest('.cart__qty').find(".quantity_selector").val());
    ...
    $(this).closest('.cart__qty').find(".quantity_selector").val(inputVal);
    $(this).closest('.cart__qty').find(".quantity_selector").change();
  ...
  // Bespoke tag
  var isBespoke = $('.template-product .main-image-wrapper').attr('data-tag');
  if(gallerySlider.length) {
    ...
    /********** PDP - Product video autoplay in mobile **********/  
    gallerySlider.on("afterChange", function (event, slick, currentSlide, nextSlide){
      var videoID = $('[data-slick-index="'+currentSlide+'"]').find('video').attr('id');
      if(videoID){
        console.log(videoID);
        var videoGridElement = document.getElementById(videoID);
        videoGridElement.play();
      }
    });
  }
  ...
  if(thumbnailSlider.length) {
    thumbnailSlider.slick({
      dots: false,
      arrows: true,
      prevArrow:"<button type='button' class='slick-prev pull-left'><svg xmlns='http://www.w3.org/2000/svg' width='24' height='24' viewBox='0 0 24 24'><defs><style>.a{fill:#fff;opacity:0;}.b{opacity:0.5;}.c{fill:#738398;}</style></defs><g transform='translate(-56 -188)'><rect class='a' width='24' height='24' transform='translate(56 188)'/><g class='b' transform='translate(58 190)'><path class='c' d='M18.747-65.045A10.075,10.075,0,0,0,28.754-75.052,10.089,10.089,0,0,0,18.737-85.059a10.079,10.079,0,0,0-10,10.007A10.081,10.081,0,0,0,18.747-65.045Zm0-15.511a.872.872,0,0,1,.638.3L23-76.631a.8.8,0,0,1,.226.549.769.769,0,0,1-.785.785.792.792,0,0,1-.569-.235l-1.315-1.334-1.079-1.295.078,2.266v5.563a.765.765,0,0,1-.8.795.765.765,0,0,1-.8-.795V-75.9l.088-2.256-1.079,1.285-1.324,1.334a.781.781,0,0,1-.569.235.759.759,0,0,1-.765-.785.748.748,0,0,1,.206-.549l3.62-3.62A.848.848,0,0,1,18.747-80.556Z' transform='translate(-8.74 85.059)'/></g></g></svg></button>",
      nextArrow:"<button type='button' class='slick-next pull-right'><svg xmlns='http://www.w3.org/2000/svg' width='24' height='24' viewBox='0 0 24 24'><defs><style>.a{fill:#fff;opacity:0;}.b{opacity:0.5;}.c{fill:#738398;}</style></defs><g transform='translate(-56 -728)'><rect class='a' width='24' height='24' transform='translate(56 728)'/><g class='b' transform='translate(58 730)'><path class='c' d='M18.747-85.059A10.075,10.075,0,0,1,28.754-75.052,10.09,10.09,0,0,1,18.737-65.045a10.079,10.079,0,0,1-10-10.007A10.081,10.081,0,0,1,18.747-85.059Zm0,15.511a.872.872,0,0,0,.638-.3L23-73.472a.8.8,0,0,0,.226-.549.769.769,0,0,0-.785-.785.792.792,0,0,0-.569.235l-1.315,1.334-1.079,1.295.078-2.266v-5.563a.765.765,0,0,0-.8-.795.765.765,0,0,0-.8.795v5.563l.088,2.256-1.079-1.285-1.324-1.334a.781.781,0,0,0-.569-.235.759.759,0,0,0-.765.785.748.748,0,0,0,.206.549l3.62,3.62A.848.848,0,0,0,18.747-69.548Z' transform='translate(-8.74 85.059)'/></g></g></svg></button>",
      infinite: false,
      speed: 500,
      vertical: true,
  ...
  function productImageFilter(variantName){
    var classLength = $(variantName).length;
    var $slider = $('.mm__product-gallery,.product-single__thumbnails');
    $slider.slick('slickUnfilter').slick('slickFilter',variantName);
  ...
  if(isBespoke == 'bespoke' || isBespoke == 'bespokering' || isBespoke == 'variant_thumbnails'){	
    var variantName = $('[name="id"] option:selected').attr("data-color");
    setTimeout(function(){
      productImageFilter(variantName);
    },100);
    $('.custom-select-options li').on('click',function(){
      setTimeout(function(){
        variantName = $('[name="id"] option:selected').attr("data-color");
  ...
  if($(window).width() < 767){
    $('p.accordion_btn_foot').on('click',function(){
  	  $(this).closest('.site-footer__item-inner').find('.accordion_text_foot').slideToggle();
...
$(document).ready(function() {
  ...
  var ajaxCartDrawer = {
    ...
    AjaxcartRender: function() {
      $.get('/cart?view=mm-ajax', function(cart) {
        var CartHtml = cart;
        var jeweliliProductCount = 0;
        $('.mm-ajaxcart').html(cart);
        $('.cart-popup-item-wrapper .cart-popup-item').each(function(){
          var productVendor = $(this).attr('data-vendor');
          var productQuantity = parseInt($(this).find('.quantity-input').val());
          /*if(productVendor.indexOf('Jewelili')>-1){
            jeweliliProductCount+=productQuantity;
          }*/
		});
        /*console.log(jeweliliProductCount);
        theme.jeweliliAddedProductInCart = jeweliliProductCount
        if(jeweliliProductCount<3){
          $('.BTGO_PopUp').css({"visibility":"visible","opacity":"1","display":"flex"});
          $("body").css("overflow","hidden").addClass('mm-popup--opened');
        }else{
        }        
        if(theme.jeweliliAddedProductInCart>0 && theme.jeweliliAddedProductInCart<2){
          $('.BTGO_PopUp [data-bfcount]').text('2');
        }
        if(theme.jeweliliAddedProductInCart>1 && theme.jeweliliAddedProductInCart<3){
          $('.BTGO_PopUp [data-bfcount]').text('1');
        }*/
    ...
    AjaxcartRenderOnChangeQuantity: function() {
      $.get('/cart?view=mm-ajax', function(cart) {
        $('.mm-ajaxcart').html(cart);
    ...
    AjaxcartChangeHandler: function(data) {
      ...
      $.ajax({
        ...
        success: function(cart) {
          ajaxCartDrawer.AjaxcartRenderOnChangeQuantity();
    ...
    AjaxcartQuantityHandler: function() {
      ...
      $(document).on('click touchstart', '.mm-ajaxcart-remove-item', function() {
  ...
  $(document).on('touchstart click', '.mm-ajaxcart-open,.site-header__cart', function(e) {
    e.preventDefault();
    ajaxCartDrawer.AjaxcartOpen();
    ajaxCartDrawer.AjaxcartRenderOnChangeQuantity();
  });

  $(document).on('touchstart click', '.mm-ajaxcart-close, .mm-ajaxcart-overlay', function(e) {
    e.preventDefault();
    ajaxCartDrawer.AjaxcartClose();
  });
  
  //AJAX ADD TO CART PDP
  $(document).on('touchstart click', '.product-form button[name="add"]', function(e) {
    e.preventDefault();
    ...
    function ajaxAdd(data) {
      $.ajax({
        ...
        success: function (data) {
          ...
          ajaxCartDrawer.AjaxcartRenderOnChangeQuantity();
          setTimeout(function () {
            $this.find("span:first-child()").removeClass("hide");
  ...
  //   Ajax cart on PLP
  $(document).on('click touchstart', '.mm-product-info button[type="submit"]', function(e) {
    e.preventDefault();
  ...
  // Stone Chart Pop Up
  $('.open_stoneChart').on('click', function(){
    $('.Stone_chart_popup').css('display','flex');
    $('body').css('overflow-y','hidden');
  });
  $('.close_stoneChart').on('click', function(){
    $('.Stone_chart_popup').css('display','none');
    $('body').css('overflow-y','auto');
  });
  // Code For custom tag Name engraving
  var isCustomProduct = $('.template-product .product-form__controls-group').attr('data-tag');
  ...
  if (isCustomProduct != undefined){
    ...
    if (color == 'White Gold' || color == 'Silver'){
      $('#colr_img').attr('src','https://cdn.shopify.com/s/files/1/0252/9620/7926/files/White_Gold.jpg?v=1612252253');
    }
    if (color == 'Yellow Gold'){ 
      $('#colr_img').attr('src','https://cdn.shopify.com/s/files/1/0252/9620/7926/files/Yellow_Gold.jpg?v=1612252253');
    }
    if (color == 'Rose Gold'){ 
      $('#colr_img').attr('src','https://cdn.shopify.com/s/files/1/0252/9620/7926/files/Rose_Gold.jpg?v=1612252253');
    }
    ...
    $('#embros_txt').keyup(function(event) {  
      var newText = event.target.value;
      $('#toChange').html(newText);
    }); 

    $('#dancing_font').click(function() {
      $('#toChange').css('font-family', 'dancing_script_otregular');
      $('#dancing_font img').css('border','2px solid #7d819a');
      $('#tinos_reg img').css('border','none');
    });

    $('#tinos_reg').click(function() {
      $('#toChange').css('font-family', 'tinosregular');
      $('#tinos_reg img').css('border','2px solid #7d819a');
      $('#dancing_font img').css('border','none');
    });
  }
  //changing font end

  // Navigation
  $('.menu__level--one').on('click', function() {
    $(this).toggleClass('open');
    $(this).next('ul').slideToggle();
    $('.menu__level--one').not(this).removeClass('open');
    $('.menu__level--one').not(this).next('ul').slideUp();
  });

  $('.menu__level--two > span').on('click', function() {
    $(this).parent('.menu__level--two').toggleClass('open');
    $(this).next('ul').slideToggle();
  });
  ...
  var homePage = {
    initBannerSlider: function() {
      var bannerSlider = $('.jewelili__banner');
    ...
    initFourBannerSlider: function() {
      var FourBannerSlider = $('.four-banner-slider');
      FourBannerSlider.slick({
        dots: false,
        arrows: true,
        prevArrow:"<button type='button' class='slick-prev'><svg class='slider_arrow_left' xmlns='http://www.w3.org/2000/svg' width='40' height='40' viewBox='0 0 48 48'><defs><style>.slider_arrow_left .a{fill:#fff;opacity:0;}.slider_arrow_left .b{fill:#292d36;}</style></defs><g transform='translate(-22 -1908)'><rect class='a' width='48' height='48' transform='translate(22 1908)'/><g transform='translate(26.931 1913)'><path class='b' d='M27.74-47.059a19.129,19.129,0,0,1-19-19,19.157,19.157,0,0,1,19.019-19,19.137,19.137,0,0,1,18.981,19A19.141,19.141,0,0,1,27.74-47.059ZM16.862-66.077a1.739,1.739,0,0,0,.577,1.267l7.134,7.116a1.678,1.678,0,0,0,1.118.447,1.535,1.535,0,0,0,1.565-1.546,1.6,1.6,0,0,0-.484-1.136L24.145-62.5l-2.533-2.142,4.489.168H37.017a1.521,1.521,0,0,0,1.583-1.6,1.531,1.531,0,0,0-1.583-1.6H26.1l-4.489.186,2.533-2.123,2.626-2.608a1.635,1.635,0,0,0,.484-1.118,1.535,1.535,0,0,0-1.565-1.546,1.578,1.578,0,0,0-1.118.428l-7.134,7.134A1.652,1.652,0,0,0,16.862-66.077Z' transform='translate(-8.74 85.059)'/></g></g></svg></button>",
        nextArrow:"<button type='button' class='slick-next'><svg class='slider_arrow_right' xmlns='http://www.w3.org/2000/svg' width='40' height='40' viewBox='0 0 48 48'><defs><style>.slider_arrow_right .a{fill:#fff;opacity:0;}.slider_arrow_right .b{fill:#292d36;}</style></defs><g transform='translate(-1210 -1907)'><rect class='a' width='48' height='48' transform='translate(1210 1907)'/><path class='b' d='M27.74-47.059a19.129,19.129,0,0,0,19-19,19.157,19.157,0,0,0-19.019-19,19.137,19.137,0,0,0-18.981,19A19.141,19.141,0,0,0,27.74-47.059ZM38.619-66.077a1.739,1.739,0,0,1-.577,1.267l-7.134,7.116a1.678,1.678,0,0,1-1.118.447,1.535,1.535,0,0,1-1.565-1.546,1.6,1.6,0,0,1,.484-1.136L31.335-62.5l2.533-2.142-4.489.168H18.464a1.521,1.521,0,0,1-1.583-1.6,1.531,1.531,0,0,1,1.583-1.6H29.379l4.489.186-2.533-2.123-2.626-2.608a1.635,1.635,0,0,1-.484-1.118,1.535,1.535,0,0,1,1.565-1.546,1.578,1.578,0,0,1,1.118.428l7.134,7.134A1.652,1.652,0,0,1,38.619-66.077Z' transform='translate(1206.19 1998.059)'/></g></svg></button>",
        infinite: true,
        speed: 1000,
        slidesToShow: 5,
        slidesToScroll: 1,
    ...
    initFiveBlocksSlider: function() {
      var OneHalfBlockSlider = $('.one-half-block-sldier');
      var FiveBlockSlider = $('.five-block-collection-slider');
      OneHalfBlockSlider.slick({
        dots: false,
        arrows: true,
        prevArrow:"<button type='button' class='slick-prev'><svg class='slider_arrow_left' xmlns='http://www.w3.org/2000/svg' width='40' height='40' viewBox='0 0 48 48'><defs><style>.slider_arrow_left .a{fill:#fff;opacity:0;}.slider_arrow_left .b{fill:#b14a6b;}</style></defs><g transform='translate(-22 -1908)'><rect class='a' width='48' height='48' transform='translate(22 1908)'/><g transform='translate(26.931 1913)'><path class='b' d='M27.74-47.059a19.129,19.129,0,0,1-19-19,19.157,19.157,0,0,1,19.019-19,19.137,19.137,0,0,1,18.981,19A19.141,19.141,0,0,1,27.74-47.059ZM16.862-66.077a1.739,1.739,0,0,0,.577,1.267l7.134,7.116a1.678,1.678,0,0,0,1.118.447,1.535,1.535,0,0,0,1.565-1.546,1.6,1.6,0,0,0-.484-1.136L24.145-62.5l-2.533-2.142,4.489.168H37.017a1.521,1.521,0,0,0,1.583-1.6,1.531,1.531,0,0,0-1.583-1.6H26.1l-4.489.186,2.533-2.123,2.626-2.608a1.635,1.635,0,0,0,.484-1.118,1.535,1.535,0,0,0-1.565-1.546,1.578,1.578,0,0,0-1.118.428l-7.134,7.134A1.652,1.652,0,0,0,16.862-66.077Z' transform='translate(-8.74 85.059)'/></g></g></svg></button>",
        nextArrow:"<button type='button' class='slick-next'><svg class='slider_arrow_right' xmlns='http://www.w3.org/2000/svg' width='40' height='40' viewBox='0 0 48 48'><defs><style>.slider_arrow_right .a{fill:#fff;opacity:0;}.slider_arrow_right .b{fill:#b14a6b;}</style></defs><g transform='translate(-1210 -1907)'><rect class='a' width='48' height='48' transform='translate(1210 1907)'/><path class='b' d='M27.74-47.059a19.129,19.129,0,0,0,19-19,19.157,19.157,0,0,0-19.019-19,19.137,19.137,0,0,0-18.981,19A19.141,19.141,0,0,0,27.74-47.059ZM38.619-66.077a1.739,1.739,0,0,1-.577,1.267l-7.134,7.116a1.678,1.678,0,0,1-1.118.447,1.535,1.535,0,0,1-1.565-1.546,1.6,1.6,0,0,1,.484-1.136L31.335-62.5l2.533-2.142-4.489.168H18.464a1.521,1.521,0,0,1-1.583-1.6,1.531,1.531,0,0,1,1.583-1.6H29.379l4.489.186-2.533-2.123-2.626-2.608a1.635,1.635,0,0,1-.484-1.118,1.535,1.535,0,0,1,1.565-1.546,1.578,1.578,0,0,1,1.118.428l7.134,7.134A1.652,1.652,0,0,1,38.619-66.077Z' transform='translate(1206.19 1998.059)'/></g></svg></button>",
        infinite: false,
        adaptiveHeight: true,
        slidesToShow: 1.1,
        slidesToScroll: 1,
    ...
    initBrandSlider: function() {
      var brandSlider = $('.four-block-banner .display-flex');
      if (brandSlider.length && $(window).width() < 768 ) {
        brandSlider.slick({
          dots: true,
          arrows: false,
          infinite: true,
    ...
    initOccasionSlider: function() {
      var occasionSlider = $('.collection_slider');
      var occasionSliderCount = parseInt($(occasionSlider).find('.collection_grid').length);
      var occasionSlidesToShow = occasionSliderCount > 3 ? 3: occasionSliderCount;
      if (occasionSlider.length) {
        occasionSlider.slick({
          dots: true,
          arrows: false,
          infinite: true,
          speed: 500,
          slidesToShow: occasionSlidesToShow,
          slidesToScroll: 1,
          autoplay: false,
          responsive: [
            {
    ...
    initCollectionSlider: function() {
      var collectionSlider = $('.jewelili__collections-column .grid');
    ...
    testimonialSlider: function() {
      var testimonialSlider = $('.image-text-blogger');
      if(testimonialSlider.length) {
        testimonialSlider.slick({
          dots: true,
          arrows: false,
          prevArrow:false,
          nextArrow:"<button type='button' class='slick-next pull-right'><img src='https://cdn.shopify.com/s/files/1/0252/9620/7926/files/Grey_arrow_b1a6237a-d7a6-451e-a64d-06bfcd1047f6.png?v=1610439867'></button>",
          infinite: true,
          speed: 500,
          slidesToScroll: 1,
          slidesToShow: 1,
    ...
    initInstagramSlider: function() {
      var collectionSlider = $('.insta-posts');
    ...
    HeaderDealsSlider: function() {
      var JeweliliDealSlider = $('.deals_slider');
      if(JeweliliDealSlider.length) {
        JeweliliDealSlider.slick({
          slidesToShow: 3,
          dots: false,
          arrows: false,
          rows:0,
	...
	/*promotionPopup: function() {
      ...
      if(popup != 'open'){
        $("#size-chart-popup").css({"visibility":"visible","opacity":"1","display":"flex"});
      }
      ...
      $(".overlay .close-popup").click(function(e){
        $(this).closest('.overlay').css({"visibility":"hidden","opacity":"0","display":"none"});
        Cookies.set('popup', 'open');
      });
    }*/
  }
  ...
  homePage.HeaderDealsSlider();
  homePage.initBannerSlider();
  homePage.initFourBannerSlider();
  homePage.initFiveBlocksSlider();
  homePage.initCollectionSlider();
  homePage.initBrandSlider();
  homePage.initOccasionSlider();
  homePage.initInstagramSlider();
  homePage.initCollectionProductSlider();
  // homePage.promotionPopup();
  homePage.testimonialSlider();
  homePage.HomeReviewSlider();


  var productDetailsPage = {
    initGallerySlider: function() {
      var recommSlider = $('.mm__recommended--items');

      //       gallerySlider

      if(recommSlider.length) {
        recommSlider.slick({
          lazyLoad: 'on-demand',
          dots: false,
          arrows: true,
    ...
    initGestureSlider: function() {
      var gestureSlider = $('.mm__gesture-slider');

      if(gestureSlider.length) {
        gestureSlider.slick({
          slidesToShow: 4,
          slidesToScroll: 1,
          infinite: false,
  ...
  productDetailsPage.initGestureSlider();
  ...
  $("button.shopify-payment-button__button.shopify-payment-button__button--unbranded").on("click",function(e){
    ...
    // Code For custom tag Name engraving
    var isCustomProduct = $('.template-product .product-form__controls-group').attr('data-tag');
    var custumRingSize = $('#ringsize').val();
    e.preventDefault();
    
    $(this).find('.checkout-text').addClass('hide');
    $(this).find('[data-loader]').removeClass('hide');
    
    var productValuePrice = $('body .product-single__meta').attr('data-product-price');
    
    var variantId = $('[name="id"]').find(":selected").val();   
    
    
    console.log(isCustomProduct);
    
    //   Add product to formdata array on click on buy now button  
    if(isCustomProduct){
      var productData = {id: variantId,quantity: 1, properties: {'Size':custumRingSize}};
    }else{
      var productData = {id: variantId,quantity: 1};
    }
    ...
    //   Add free product to formdata array on click on buy now button, if price greater than $ 190
    /*if(productValuePrice >= 19000){
      var data = {id: 40579271327929, quantity: 1};
      formData.push(data);
    }*/
    
    //  code to check cart quantity
    var cartQuantity = parseInt($('#CartCount [data-cart-count]').text());
  ...
  //       $(".accordion_heading").removeClass('open');
  //       $(".tab_content").css('display','none');
  //       $('.accordion_heading').on('click', function() {
  //         $('.accordion_heading').not(this).removeClass('open');
  //         $(this).addClass('open');
  //         $('.accordion_heading').not(this).next('.tab_content').slideUp();
  //         $(this).next('.tab_content').slideDown();
  //       })
  //     }
  $(".accordion_heading").removeClass('open');
  $(".accordion_heading:first-of-type()").addClass('open');
  $('.accordion_heading').on('click', function(e) {
    $('.accordion_heading').not(this).removeClass('open');
    $(this).toggleClass('open');
    $('.accordion_heading').not(this).next('.tab_content').slideUp();
  ...
  // PDP Customized Variant Select  
  $('.custom-dropdown-selected').on('click', function() {
    $('.custom-dropdown-selected').not(this).removeClass('active');
    $('.custom-dropdown-selected').not(this).next('.custom-select-options').hide();
    $(this).addClass('active');
    $(this).next('.custom-select-options').show();
  });

  $('.custom-select-options li').on('click', function() {    
    var dataOptionVal = $(this).data('option-value');
    var dataContent = $(this).html();
    $(this).parents('.product-form__item').find('.single-option-selector').val(dataOptionVal).trigger('change');
    $(this).parents('.product-form__item').find('.custom-dropdown-selected').html(dataContent);
  });

  // Select custom li for gemstone and metal js Start
  $('.Color_dropdown .custom_li').on('click', function() {
    $('.Color_dropdown .custom_li').removeClass('selected');
    $(this).addClass('selected');
  });

  $('.Gemstones_dropdown .custom_li').on('click', function() { 
    $(".Gemstones_dropdown .custom_li").removeClass('selected');
    $(this).addClass('selected');
  });
  // Select custom li for gemstone and metal js End

  $(document).mouseup(function(e) {
    var container = $('.custom-dropdown-selected');
    if (!container.is(e.target) && container.has(e.target).length === 0) {
      container.removeClass('active');
      container.next('.custom-select-options').hide();
    }
  });

  // Size Guide Popup
  $(".open_popup").click(function () {
    $(this).next(".popup_main").children(".popup_body").addClass("popup_body_show");
  });
  ...
  // Faq accordian
  $(".faq-block .faq-title").on("click",function(){
    $(this).closest(".faq-block").toggleClass("active");
    $(this).closest(".faq-block").find(".faq-description").slideToggle();
  });

  // Search Functionality
  var ApplyButton = $('#search-items');
  ApplyButton.on('click', function(e) {
    e.preventDefault();
    if($('.select-styled.selected').length > 0){
      var Self = $(this),
          CollectionUrl = '/collections/all',      
          FilterItems = $('.search_form select');
      var Filters = [];     
      FilterItems.each(function() {
        Filters.push($(this).val());	
      });
      console.log(Filters);
      document.location.href = CollectionUrl + '?q=' + Filters.join(',');
    } else{
...
$('[data-section-type="jewelili-search-section"] select, [data-section-type="jewelili-ask-jenie"] select').each(function(){
  ...
  $listItems.click(function(e) {
    ...
    $this.parents('.select').find('.select-options').attr('data-selected', $this.val());
    $('#search-items').css('opacity','1');
    if($this.parents('.select').find('.select-styled').hasClass('selected')){
...
$(document).ready(function() {
  ...
  $('#CustomerRegisterForm input').on('click', function() {
    $(this).removeClass('input--error').next('[data-form-error]').hide();
  });

  $('#CustomerLoginForm .btn--primary').on('click', function(e) {
  ...
  $('#CustomerLoginForm input').on('click', function() {
    $(this)
    .removeClass('input--error')
    .next('[data-form-error]').hide();
  });
  ...
  $(".overlay .close-popup, .overlay").click(function(e){
  ...
  /********** code to make month sticky and active on scoll **********/
  var monthPageUrl = window.location.href;
  if(monthPageUrl.indexOf('birthstones-jewelry-by-months')>-1){
    function stickBirthMonth(){
      var windowOffest = $(window).scrollTop();
      var monthBar = $(".month-sticky-height").offset().top;
      console.log(monthBar,windowOffest);
      $('.month-sticky-height').css("height",$(".month-wrapper").height);
      if(windowOffest>monthBar){
        $(".month-wrapper").addClass('sticky');
      }else{
        $(".month-wrapper").removeClass('sticky');
      }
    }
  ...
  /********** code to make month sticky and active on scoll -- END **********/
  ...
  $(document).on("click",".shop-by-price .feature-row__image-wrapper",function(e){
    var child = $(this).find('img').data('child');
    $('.filter-option-price .filter-content li:nth-child('+child+')').trigger('click');
    var hash = $('.shop-by-price');
    $('html, body').animate({
      scrollTop: $(hash).offset().top + 100
    }, 800);
  });
  
  $(document).on("click",".mother-category-index .txt-img-block",function(e){
    e.preventDefault();
    var child = $(this).data('child');
    $('.filter-option-tag [data-tag="Tag-4"] li:nth-child('+child+')').trigger('click');
    var hash = $('.shop-by-price');
    $('html, body').animate({
      scrollTop: $(hash).offset().top + 100
    }, 800);
  });
  // Holiday sale collection four block filter
  $(document).on("click",".four-block-banner .single-grid-wrapper",function(e){
    e.preventDefault();
    var child = $(this).data('child');
    $('.filter-option-tag [data-tag="Tag-6"] li:nth-child('+child+')').trigger('click');
    var hash = $('.collectionTitle');
    $('html, body').animate({
...
$(document).ready(function() {
  if ($(window).width() < 768) {
    $('.blogs-nav .nav').hide();
    $('.blogs-nav .mobile-nav-btn').click(function(e) {
      e.preventDefault();
      $('.blogs-nav .nav').slideToggle();
      $('.blogs-nav').toggleClass('nav-open');
    });
  }
  ...
  var myVar2 = setInterval(giveaway, 500);
  var count;
  function giveaway() {
    count += 1;
    var KlaviyoLen = $('.klaviyo-form-XiMeEQ').children().length;
    if(KlaviyoLen != 1){
      $('.klaviyo-form-XiMeEQ').siblings('[data-loader]').removeClass('hide');
    }else{
      $('.klaviyo-form-XiMeEQ').siblings('[data-loader]').addClass('hide');
    }
  }  
});