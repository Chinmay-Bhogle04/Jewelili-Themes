...
$(document).ready(function() {
  ...
  $("button.shopify-payment-button__button.shopify-payment-button__button--unbranded").on("click",function(e){
    ...
    $(this).find('[data-loader]').removeClass('hide');
    var productValuePrice = $('.product-single__meta').data('product-price');
    var formData = $(this).closest('form').serializeArray();
    console.log(isCustomProduct);
    //       var data = {"id": 32683768578102,"quantity": 1}
    //       formData.push(data);
    try{
      var engravingText = $('.product-form #embros_txt').val().length;
      console.log(engravingText);
    }catch(e){
      console.log(e);
    }
    ...
    if(isCustomProduct != undefined){
      ...
      if(productValuePrice>=19900){
        window.location.href = "/cart/"+variantId+":1,32683768578102:1";
      }else{
