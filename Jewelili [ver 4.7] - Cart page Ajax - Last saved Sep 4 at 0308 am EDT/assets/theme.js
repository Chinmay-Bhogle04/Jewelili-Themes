...
$(document).ready(function() {
  ...
  var ajaxCartDrawer = {
    ...
    AjaxcartRender: function() {
      $.get('/cart?view=mm-ajax', function(cart) {
        ...
        if(FreeProductPrice >= 20000 && !hasFreeProduct){
          data = {
            "id": 32683768578102,
            "quantity": 1
          }
          ...
          //           jQuery.post('/cart/update.js', {updates: {32683768578102: 0}});
        }
  ...
  // Ajax Cart Page JS Start
  var ajaxCartPage = {
    AjaxcartPageRender: function() {
      $.get('/cart?view=mm-cart-page-ajax', function(cart) {
        $('.cart-page-ajax').html(cart);
        // freeProduct gift code start
        var FreeProductPrice = $('.cart-subtotal #free-gift-product200').html();
        var hasFreeProduct = false;
        
        $('.cart-item-wrapper .cart-item').each(function(){
          var itemVariant = $(this).data('variant-id');
          if(itemVariant == '32683768578102'){
            hasFreeProduct = true;
          }
        ...
        console.log('Free price',FreeProductPrice,hasFreeProduct);
    ...
    AjaxcartPageChangeHandler: function(data) {
      $.ajax({
        url: '/cart/change.js',
        dataType: 'json',
        cache: false,
        ...
        success: function(cart) {
          console.log(cart);
          ajaxCartPage.AjaxcartPageRender();
          var CartCount = cart.item_count;
          if (CartCount > 0) {
            $('[data-cart-count]').removeClass('hide');	
          } else {
            $('[data-cart-count]').addClass('hide');
          }
          $('[data-cart-count]').html(CartCount);
          CartCount < 2?($('.cart-popup__heading span').text(CartCount+' item')):($('.cart-popup__heading span').text(CartCount+' items'));
        }
    ...
    AjaxcartPageQuantityHandler: function() {
      ...
      $(document).on('change', '.cart__qty-input', function() {
        ...
        console.log(params);
        ajaxCartPage.AjaxcartPageChangeHandler(params);
      })
      $(document).on('click', '.button_holders .remove', function() {
        var RemoveLine = parseInt($(this).attr('data-line')),
            params = {
              quantity: 0,
              line: RemoveLine
  ...
  ajaxCartPage.AjaxcartPageRender();
  ajaxCartPage.AjaxcartPageQuantityHandler();
  
  // Ajax Cart Page JS End
